...
%
% If set before nemoStart is called, the backend is configured to run with
% STDP.  It will gather synapses modification statistics continously. To update
% the synapses using the accumulated values, call nemoApplySTDP.
%
...
% postsynaptic fired.
%
% The length of each vector specify the time window during which STDP has an
% effect.
%
...
% However, the STDP function can be configured differently. Any cycle for which
% the function has a positive value is a potentation cycle, and conversely,
% negative values indicate depression. This results in two distinct regions of
% the STDP window. These regions are treated differently. For any postsynaptic
% firing only the spike whose arrival is closest in time to the firing is used
% to update the potentation/depression statistics.
%
