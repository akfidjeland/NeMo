...
# Includes
INCLUDES  += -I. -I$(CUDA_INSTALL_PATH)/include -I$(COMMONDIR)/inc $(EXTINCLUDES)
...
ifeq ($(dbg),1)
	COMMONFLAGS += -g
	NVCCFLAGS   += -D_DEBUG
	COMMONFLAGS += -DDEVICE_ASSERTIONS
	BINSUBDIR   := debug
	# LIBSUFFIX   := D
else 
	COMMONFLAGS += -O3 
...
# Libs
LIB       := -L$(CUDA_INSTALL_PATH)/lib -L$(CUDA_SDK_PATH)/lib -L$(LIBDIR) -L$(COMMONDIR)/lib/$(OSLOWER) $(EXTLIBS)
ifeq ($(USEDRVAPI),1)
...
ifneq ($(STATIC_LIB),)
	...
	LINKLINE  = $(LINK) -o $(TARGET) $(LINKFLAGS) $(OBJS) $(LIB)
endif
...
# Need shared memory atomics
NVCCFLAGS += -arch=sm_12 # fails if using -arch=compute12, TODO: investigate why
...
cuda_clean:
	...
	-$(VERBOSE)find $(LIBDIR) -iname '*.a' -delete
	-$(VERBOSE)rm -f $(CUBINS)
