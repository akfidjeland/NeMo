...
This accumulation takes place every cycle,
	but the modification of the weight only takes place when explicitly requested.
%Either the user specifies a fixed frequency at which STDP is applied,
%	or the user explicitly issues a command to apply it. 
% In the latter case the user can also supply a reward signal which is
% multiplied into the accumulated weight change for all neurons.
...
In the simulation on of these minima will be one cycle,
...
Synaptic weights are never potentiated beyond the user-specified a maximum weight. 
Additionally, synapses are never depressed below weight zero. 
They can thus be deactivated, but will not change from excitatory to inhibitory.
This latter clamping rule, means that in the current version of \nemo\ 
plasticity of inhibitory synapses is not well supported.
...
nemoConnect;
nemoSetBackend('axel01.doc.ic.ac.uk');
